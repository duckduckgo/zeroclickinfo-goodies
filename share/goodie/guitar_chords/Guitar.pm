...
my $SIZE = 1.5;
my @lines = (
    [5*$SIZE,15*$SIZE,46*$SIZE,15*$SIZE],
    [5*$SIZE,21*$SIZE,46*$SIZE,21*$SIZE],
    [5*$SIZE,27*$SIZE,46*$SIZE,27*$SIZE],
    [5*$SIZE,33*$SIZE,46*$SIZE,33*$SIZE],
    [5*$SIZE,39*$SIZE,46*$SIZE,39*$SIZE],
    [5*$SIZE,45*$SIZE,46*$SIZE,45*$SIZE],
...
sub generate {
    ...
    if ($fret_max > 5) {
        $im->filledRectangle(4*$SIZE, 15*$SIZE, 6*$SIZE, 45*$SIZE, $bgcolor);
        ...
        for my $n (0..4) { $im->string(GD::Font->Small, $n * (8 * $SIZE) + 10, 47*$SIZE, $fret_num + 1, $color); $fret_num++; } } my $i = 0; for my $fret (@frets) { if (lc $fret eq 'x') { $im->line(0*$SIZE, (14 + 6 * $i)*$SIZE, 2*$SIZE, (16 + 6 * $i)*$SIZE, $color); $im->line(2*$SIZE, (14 + 6 * $i)*$SIZE, 0, (16 + 6 * $i)*$SIZE, $color);
        }
        elsif ($fret > 0) {
            $im->filledRectangle(
                (9  + 8 * ($fret - 1))*$SIZE,
    ...
    $im->stringFT($color, "./font.ttf", 9, 0, 5, 16, $chord);
    return $im;
}
